...
Документы курса: **`project/ТЗ_0.txt`**, **`project/Критерии_оценки_fastapi_llm.txt`** и прочие файлы в **`project/`**.
...
1. Перейти в каталог приложения и скопировать пример переменных окружения:
   ...
   Локальный файл **`project\.env`** (если есть) приложению по умолчанию не нужен — при необходимости скопируйте нужные ключи в **`llm_p\.env`**.

2. Отредактировать **`llm_p\.env`**: задать `JWT_SECRET`, при необходимости `OPENROUTER_API_KEY` и прочие поля из примера.
